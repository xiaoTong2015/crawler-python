...
#config = load_json()
# 网站根路径
base_url = config[0]['baseURL']
# 最开始请求的页面url地址
url = config[0]['requireURL']
link_list_new = []
tb_all = []
# 表头
tb_title = config[0]['tbHead']
#tb_all.append(','.join(tb_title))
a = webdriver.Firefox(executable_path=config[0]['executable_Path'])
...
def get_newlink_list(bs):
    link_list = bs.find_all(title=re.compile(config[0]['dataTitle']))
    for link in link_list:
        matchObj = re.search(config[0]['analysis_formulas']['title_regular'], str(link), re.M | re.I)
        if matchObj:
...
for url_item in link_list_new:
    ...
    html_p = bs_item.select(config[0]['select_tag'][0])
    if not html_p:
        html_p = bs_item.select(config[0]['select_tag'][1])
